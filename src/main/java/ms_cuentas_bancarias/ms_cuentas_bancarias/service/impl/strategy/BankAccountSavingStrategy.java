...
import ms_cuentas_bancarias.ms_cuentas_bancarias.utils.SubTypeClient;
...
@Component
public class BankAccountSavingStrategy implements BankAccountStrategy {

    private final BankAccountRepository bankAccountRepository;
    private final TransactionRepository transactionRepository;
    private final CommonStrategy commonStrategy;

    private static final Logger LOGGER = LoggerFactory.getLogger(BankAccountSavingStrategy.class);

    private static final Integer MAX_LIMIT_TRANSACTION_PER_MONTH = 50;
    private static final BigDecimal PENALTY_AMOUNT = BigDecimal.valueOf(5.00);
    ...
    private Mono<Void> validateOnlyOneCreditByType(final AccountRequest accountRequest,
        ...
        if (clientResponse.getType().equals(TypeClient.PERSONAL)) {
            if (clientResponse.getSubType().equals(SubTypeClient.VIP)) {
                return commonStrategy.validateActiveCreditCard(clientResponse.getId())
                        .then(commonStrategy.validateOneHaveOneTypeAccountForPersonalClient(accountRequest));
            }
    ...
    private Mono<TransactionResponse> processTransaction(
        ...
        return commonStrategy.validateAndRetrieveAccount(id, transactionRequest, transactionType)
                .flatMap(bankAccount -> commonStrategy.countTransactionsThisMonth(id)
                        .flatMap(transactionCount -> validateAndProcessTransaction(transactionRequest, transactionType,
                                bankAccount, transactionCount)));
    }

    private Mono<TransactionResponse> validateAndProcessTransaction(final TransactionRequest transactionRequest,
            final TransactionType transactionType, final Account bankAccount, final Integer transactionCount) {
        boolean exceededNumberOfTransactionsPerMonth = transactionCount > MAX_LIMIT_TRANSACTION_PER_MONTH;
        BigDecimal commision = exceededNumberOfTransactionsPerMonth ? PENALTY_AMOUNT : BigDecimal.ZERO;

        if (transactionType == TransactionType.DEPOSIT) {
            bankAccount.setBalance(bankAccount.getBalance().add(transactionRequest.getAmount()).subtract(commision));
        } else if (transactionType == TransactionType.WITHDRAWAL) {
            bankAccount
                    .setBalance(bankAccount.getBalance().subtract(transactionRequest.getAmount()).subtract(commision));
        }
        ...
        return bankAccountRepository.save(bankAccount)
                .flatMap(updatedAccount -> {
                    ...
                    transaction.setComission(commision);
    ...
    public Mono<TransactionResponse> transfer(final TransferRequest transferRequest) {
        ...
        return validateTransferAmount(transferRequest)
                ...
                .flatMap(tuple -> {
                    ...
                    return validateTransactionLimits(transferRequest, targetAccount)
                            .flatMap(comision -> processTransfer(sourceAccount, targetAccount,
                                    transferRequest.getTransactionRequest(), comision));
                });
    }
    ...
    private Mono<BigDecimal> validateTransactionLimits(final TransferRequest transferRequest,
            final Account targetAccount) {
        Mono<BigDecimal> validateSourceTransactions = commonStrategy
                .countTransactionsThisMonth(transferRequest.getSourceAccountId())
                .map(transactionCountSource -> {
    ...
    private Mono<TransactionResponse> processTransfer(
            ...
            final Account targetAccount,
            final TransactionRequest transactionRequest, final BigDecimal comission) {

        sourceAccount
                .setBalance(sourceAccount.getBalance().subtract(transactionRequest.getAmount()).subtract(comission));
        ...
        withdrawalTransaction.setComission(comission);
