...
public class BankAccountCurrentStrategy implements BankAccountStrategy {

    private final BankAccountRepository bankAccountRepository;
    private final TransactionRepository transactionRepository;
    private final CommonStrategy commonStrategy;

    private static final Logger LOGGER = LoggerFactory.getLogger(BankAccountCurrentStrategy.class);
    private static final int MAX_LIMIT_TRANSACTION_PER_MONTH = 50;
    ...
    private Mono<Void> validateOnlyOneCreditByType(final AccountRequest accountRequest,
        ...
        if (clientResponse.getType().equals(TypeClient.BUSINESS)
                && clientResponse.getSubType().equals(SubTypeClient.PYME)) {
            return commonStrategy.validateActiveCreditCard(accountRequest.getClientId());
        }
    ...
    private Mono<TransactionResponse> processTransaction(
        ...
        return commonStrategy
                .validateAndRetrieveAccount(id, transactionRequest, transactionType)
                .flatMap(account -> updateAccountAndSaveTransaction(account, transactionRequest, transactionType));
    }
